...
from PyQt5.QtWidgets import QDialog, QWidget, QGroupBox, QLineEdit, QFormLayout, QCheckBox
...
class GeneralBridge(QDialog, uic.loadUiType('statsu/widgets/analysis/general_bridge.ui')[0]):
    def __init__(self, target: Callable, parent: Optional[QWidget] = None) -> None:
        ...
        for arg, val in zip(
            params,
            [None for _ in range(len(params) - len(spec.defaults))] + list(spec.defaults)  # type: ignore
        ):
            if type(val) == bool:
